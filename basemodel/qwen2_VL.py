...
class PatchEmbed(nn.Module):
    def __init__(
        ...
        self.proj = nn.Conv3d(in_channels,embed_dim,kernel_size=kernel_size,bias=False)
...
# Copied from transformers.models.llama.modeling_llama.repeat_kv - KV Cache
...
class VisionAttention(nn.Module):
    def __init__(self, config:Qwen2VLVisionConfig) -> None:
        ...
        self.sacling = self.head_dim**-0.5
        self.config = config
...
class Qwen2VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2VLTextConfig, layer_idx: Optional[int] = None):
        ...
        self.v_peoj = nn.Linear(self.hidden_size,self.num_key_value_heads * self.head_dim,bias=True)
        self.o_proj = nn.Linear(self.num_heads * self.head_dim,self.hidden_size,bias=False) # bias细节
...
# 本质上是一个ViT，负责将像素转换成向量 - Vision Encoder
# Qwen2VL中的vision部分
...
# Qwen2 LLM，负责文本处理和最终的逻辑推理 - Vision Encoder
# Qwen2VL中的language model部分
     ...
     self,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            # 用计算好的 image_embeds 替换掉那些占位符的向量
