...
"""
   VisionEncoderDecoder本身不是一个特定的模型,而是Hugging Face transformers库中一种模型架构模式。
   设计的目的是将任何一个视觉模型和任何一个序列生成模型结合在一起。
   
   在smallcap中的作用：
        1. 将一个预训练的视觉编码器（CLIP Vision Model）和一个预训练的语言解码器（GPT-2）连接起来
        2. 规定了数据如何从编码器流向解码器，将图像经过编码器后得到的特征表示（encoder_hidden_states），作为额外信息提供给解码器。
        3. 通过在解码器的每一层中加入交叉注意力（Cross-Attention）机制，它允许解码器在生成文本的每一步，都能“回顾”和“关注”图像的特定部分。
   
   总体来说，VisionEncoderDecoder就是让只会处理文字的GPT-2能够看到图像理解图像内容

   对于图像特征与检索描述特征的融合发生在解码器（GPT-2）的内部 - 主要在交叉注意力层；自注意力更多只关注文本信息本身。
...
"""
图像是通过颜色和亮度在二位空间上组合而成的视觉表示，数字图像是以矩阵形式存储的
其中每个元素代表一个像素-piexl

数字图像可以被视为一个矩阵，其中的每一个元素对应一个像素，对于灰度图像矩阵时二维矩阵，其中的每个元素表示该像素的亮度值

对于彩色图像，通常使用三个二维矩阵来表示图像的三个颜色通道

VisionEncoderDecoderModel类的核心作用:将两个完全独立的模型连接起来
    1. 一个是视觉模型：“看懂”一张图片，并将其转换成计算机能理解的数字信息特征
    2. 一个是语言模型：根据给定的信息，像人一样“写出”一段通顺的文字

VisionEncoderDecoderModel类的主要输入：
    1. 图像 pixel_values: 将描述的图片转换为向量数据
    2. 文本ID decoder_input_ids / labels: 训练时是图像的正确答案，模型如何学习这段文字；生成时是一个起始信号

VisionEncoderDecoderModel类的主要输出：
    1. 训练时输出loss损失值
    2. 推理时生成文本
    
    相当于把 GPT-2 和 CLIP模型 融合在了一起，推理的时候在VisionEncoderDecoder
    内部调用CLIP和GPT-2，通过交叉注意力机制融合视觉特征辅助语言模型自回归生成文本
