...
# 明确使用VisionEncoderDecoderModel的目标是将检索得到的文本数据和经过CLIP编码后提取到的图像特征进行
class VisionEncoderDecoderModel(PreTrainedModel, GenerationMixin):
    # VisionEncoderDecoderModel是一个通用模型类，它将实例化一个Transformer架构
    # 基础的视觉模型作为编码器和解码器
    # 使用transformers.AutoModel.from_pretrained加载编码器
    # 使用transformers.AutoModelForCausalLM.from_pretrained*加载解码器
    r"""
